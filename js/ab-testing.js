/**
 * A/B Testing Framework for Pet Anxiety Guide
...
(function(window) {
  ...
  const STORAGE_KEY = 'pag_ab_tests';
  const EVENTS_KEY = 'pag_ab_events';
  ...
  function initTests() {
    ...
    ABTest.run('cta-color', ['green', 'orange', 'purple'], {
      'green': function() {
        // Default - do nothing
      },
      'orange': function() {
        ...
        ctas.forEach(cta => {
          cta.style.background = '#F97316'; // Tailwind orange-500
          cta.addEventListener('mouseenter', function() { this.style.background = '#EA580C'; });
          cta.addEventListener('mouseleave', function() { this.style.background = '#F97316'; });
        });
      },
      'purple': function() {
        const ctas = document.querySelectorAll('button[type="submit"], .cta-button, a.bg-emerald-500, a.bg-green-500');
        ctas.forEach(cta => {
          cta.style.background = '#8B5CF6'; // Tailwind violet-500
          cta.addEventListener('mouseenter', function() { this.style.background = '#7C3AED'; });
          cta.addEventListener('mouseleave', function() { this.style.background = '#8B5CF6'; });
        });
      }
    });

    // Test 2: CTA Button Text
    ABTest.run('cta-text', ['assess', 'check', 'free'], {
      'assess': function() {
        // Default - do nothing
      },
      'check': function() {
        ...
        ctas.forEach(cta => {
          if (cta.textContent.toLowerCase().includes('assess') || cta.textContent.toLowerCase().includes('calculate')) {
            cta.textContent = 'Check My Pet\'s Score';
          }
        });
      },
      'free': function() {
        ...
        ctas.forEach(cta => {
          if (cta.textContent.toLowerCase().includes('assess') || cta.textContent.toLowerCase().includes('calculate')) {
            cta.textContent = 'Free Assessment';
          }
        });
      }
    });
